			    ...
			    int desc_len)
{
	...
	if (end - (void *)ext_hdr >= 8) {
		...
		err = do_parse_extensions(vi, &ext_hdr, end, unauth_ext_count,
					  false);
		if (err)
			return err;
	}
				     ...
				     int desc_len)
{
	...
	if (desc->log_tree_blocksize != desc->log_data_blocksize) {
		pr_warn("Unimplemented case: data (%u) and tree (%u) log_blocksizes differ\n",
			desc->log_data_blocksize, desc->log_tree_blocksize);
		return -EINVAL;
	}
			       ...
			       int nr_desc_pages, u8 *measurement)
{
	struct ahash_request *req;
	DECLARE_CRYPTO_WAIT(wait);
	struct scatterlist sgl[MAX_DESCRIPTOR_PAGES];
	...
	sg_init_table(sgl, nr_desc_pages);
	...
	for (i = 0; i < nr_desc_pages && remaining; i++) {
		...
		sg_set_page(&sgl[i], desc_pages[i], len, offset);
		remaining -= len;
		offset = 0;
	}
	...
	ahash_request_set_callback(req, CRYPTO_TFM_REQ_MAY_SLEEP |
				   CRYPTO_TFM_REQ_MAY_BACKLOG,
				   crypto_req_done, &wait);
	ahash_request_set_crypt(req, sgl, measurement, desc_auth_len);
			...
			int nr_desc_pages)
{
	...
	pr_warn("FILE CORRUPTED (actual measurement differs from signed measurement): "
...
{
	return inode->i_sb->s_vop->read_metadata_page(inode, index);
}
