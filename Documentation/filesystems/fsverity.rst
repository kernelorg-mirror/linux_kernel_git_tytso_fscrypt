...
size converges to approximately 1/127 of the original file size.
...
For all filesystems, CONFIG_FS_VERITY must be enabled in the kernel
config for fs-verity support to be available.
...
The filesystem must have been formatted with ``-O verity``, or had
``tune2fs -O verity`` run on it.  These require e2fsprogs v1.44.5 or
...
The filesystem must have been formatted with ``-O verity``.  This
requires f2fs-tools v1.11.0 or later.
...
cached and checked -- since only 1 in 128 blocks during a sequential
read will need a new hash page.  However, random reads perform worse.
